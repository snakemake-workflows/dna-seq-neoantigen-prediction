...
rule mhcflurry:
	...
	output:
		"mhcflurry/{tumor}_{normal}/{chr}/output.{group}.csv"
	log:
		"logs/mhcflurry/{tumor}_{normal}/{chr}/log.{group}.txt"
	conda:
		"../envs/mhctools.yaml"
	run:
...
rule parse_wt_mhc_out:
	input:
		expand("netMHCpan/{{tumor}}_{{normal}}/{chr}/{{tumor}}_{{normal}}.{chr}.{{group}}.xls", chr=CHROMOSOMES)
	output:
		"netMHCpan/{tumor}_{normal}/{tumor}_{normal}.mhc.{group}.tsv"
...
rule parse_mhcflurry:
	input:
		expand("mhcflurry/{{tumor}}_{{normal}}/{chr}/output.{{group}}.csv", chr=CHROMOSOMES)
	output:
		"mhcflurry/{tumor}_{normal}/mhcflurry.{group}.tsv"
	wildcard_constraints:
		group="wt|mt"
	conda:
		"../envs/xsv.yaml"
	shell:
		"xsv cat rows -d ',' {input} | xsv table -d '\t' | cut --complement -f2,7,8 > {output}"
...
rule mhcflurry_table:
	input:
		info="microphaser/info/{tumor}_{normal}/filtered/{tumor}_{normal}.tsv",
		mt="mhcflurry/{tumor}_{normal}/mhcflurry.mt.tsv",
		wt="mhcflurry/{tumor}_{normal}/mhcflurry.mhc.wt.tsv"
	output:
		"mhcflurry/{tumor}_{normal}.tsv"
	script:
